# Shared implementation for docker/*/install_cluster.sh on Debian/Ubuntu-based images.
# Sourced after SCRIPT_DIR, MASTER_CONTAINER, NODE2_CONTAINER, NODE3_CONTAINER,
# and LME_CLUSTER_LABEL are set by the wrapper.

if [[ -z "${MASTER_CONTAINER:-}" || -z "${NODE2_CONTAINER:-}" || -z "${NODE3_CONTAINER:-}" || -z "${LME_CLUSTER_LABEL:-}" || -z "${SCRIPT_DIR:-}" ]]; then
    echo "install_cluster_debian_common.sh: SCRIPT_DIR, MASTER_CONTAINER, NODE2_CONTAINER, NODE3_CONTAINER, and LME_CLUSTER_LABEL must be set before sourcing" >&2
    exit 1
fi
...
install_python_on_nodes() {
    echo -e "${YELLOW}Installing Python on cluster nodes...${NC}"
    for container in "$NODE2_CONTAINER" "$NODE3_CONTAINER"; do
        echo -e "  Installing Python on ${container}..."
        docker_exec "$container" "apt-get update && apt-get install -y python3 python3-apt"
        echo -e "  ${GREEN}✓${NC} Python installed on ${container}"
    done
}
