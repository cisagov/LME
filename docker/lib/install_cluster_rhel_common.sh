# Shared implementation for docker/*/install_cluster.sh on RHEL-family (dnf) images.
# Sourced after SCRIPT_DIR, MASTER_CONTAINER, NODE2_CONTAINER, NODE3_CONTAINER,
...
case "${LME_CLUSTER_ENV_PROFILE}" in
    extended|simple) ;;
    *)
        echo "install_cluster_rhel_common.sh: LME_CLUSTER_ENV_PROFILE must be 'extended' or 'simple'" >&2
        exit 1
        ;;
esac
...
echo -e "${GREEN}=== LME Docker ${LME_CLUSTER_LABEL} Cluster Installer ===${NC}"
...
create_environment_file() {
    ...
    local master_ip
    if [ "${LME_CLUSTER_ENV_PROFILE}" = "extended" ]; then
        master_ip=$(docker_exec "$MASTER_CONTAINER" "ip -4 -o addr show eth0 2>/dev/null | awk '{print \$4}' | cut -d/ -f1")
        if [ -z "$master_ip" ]; then
            master_ip=$(docker_exec "$MASTER_CONTAINER" "hostname -I 2>/dev/null | awk '{print \$1}'")
        fi
        if [ -z "$master_ip" ]; then
            master_ip=$(docker_exec "$MASTER_CONTAINER" "ip -4 -o addr show | grep -v '127.0.0.1' | head -1 | awk '{print \$4}' | cut -d/ -f1")
        fi
        if [ -z "$master_ip" ]; then
            master_ip=$(docker_exec "$MASTER_CONTAINER" "echo \$HOST_IP")
        fi
        if [ -z "$master_ip" ]; then
            master_ip="127.0.0.1"
        fi
    ...
    if [ "${LME_CLUSTER_ENV_PROFILE}" = "extended" ]; then
        # Remove stale /opt/lme/lme-environment.env so base role copies fresh from config
        docker_exec "$MASTER_CONTAINER" "rm -f /opt/lme/lme-environment.env"
    fi
...
echo ""
echo -e "${GREEN}=== ${LME_CLUSTER_LABEL} Cluster Setup Complete ===${NC}"
echo ""
