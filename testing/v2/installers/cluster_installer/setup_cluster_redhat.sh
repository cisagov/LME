...
# Cluster Installer Setup Script (Red Hat Enterprise Linux 9 on Azure)
# Run from: testing/v2/installers/cluster_installer
# Provisions RHEL 9 VMs and sets up a multi-node LME cluster for testing
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        -h|--help)
            ...
            echo ""
            echo "Same as setup_cluster.sh but provisions RHEL 9 VMs (Azure --use-rhel)"
            echo "and uses dnf / nfs-utils on cluster nodes."
            echo ""
            echo "OPTIONS:"
...
echo -e "${GREEN}=== LME Cluster Installer (RHEL 9) ===${NC}"
...
echo "  BRANCH: $BRANCH"
echo "  OS_DISK_SIZE_GB: ${OS_DISK_SIZE_GB:-300}"
...
# Must match build_azure_linux_network.py --os-disk-size-gb default when unset
OS_DISK_SIZE_GB="${OS_DISK_SIZE_GB:-128}"
...
for tool in jq sshpass; do
    if ! command -v $tool &> /dev/null; then
        ...
        echo -e "${YELLOW}Install with: sudo apt-get install -y $tool${NC} (Debian/Ubuntu) or sudo dnf install -y $tool (Fedora/RHEL)${NC}"
        exit 1
    fi
done
...
# Build the cluster (RHEL 9 images on Azure)
echo -e "${YELLOW}Building RHEL 9 cluster with $CLUSTER_SIZE nodes (OS disk ${OS_DISK_SIZE_GB} GB each)...${NC}"
...
# RHEL Azure images do not include git by default
echo -e "${YELLOW}Installing git on master...${NC}"
ssh "${LME_USER}@${MASTER_IP}" "sudo dnf install -y git"
echo -e "${GREEN}Git installed on master${NC}"
...
# Extend LVM/filesystems on each node so the OS uses the full Azure OS disk
# (RHEL images often leave unused space until expand_rhel_disk.sh runs).
LOCAL_REPO_ROOT="$(cd "$SCRIPT_DIR/../../../.." && pwd)"
EXPAND_SCRIPT="${LOCAL_REPO_ROOT}/scripts/expand_rhel_disk.sh"
if [ ! -f "$EXPAND_SCRIPT" ]; then
    echo -e "${RED}Error: RHEL expand script not found: $EXPAND_SCRIPT${NC}"
    exit 1
fi
echo -e "${YELLOW}Expanding OS disk usage on all Linux nodes (after clone)...${NC}"
for IP in $(jq -r '.linux_vms[].ip_address' "${RESOURCE_GROUP}.machines.json"); do
    echo "  Running expand_rhel_disk.sh on $IP ..."
    scp -o StrictHostKeyChecking=no "$EXPAND_SCRIPT" "${LME_USER}@${IP}:/tmp/expand_rhel_disk.sh"
    ssh -o StrictHostKeyChecking=no "${LME_USER}@${IP}" \
        "sudo bash /tmp/expand_rhel_disk.sh --yes && rm -f /tmp/expand_rhel_disk.sh"
done
echo -e "${GREEN}Disk expansion complete on all Linux nodes${NC}"
...
ssh "${LME_USER}@${MASTER_IP}" "sudo dnf install -y jq"
...
if [ "$SKIP_NFS" = "true" ]; then
    ...
    MASTER_SNAPSHOT_STORAGE="/var/lib/lme/es-snapshots"
    for ip in $(jq -r '.linux_vms[].private_ip' "${RESOURCE_GROUP}.machines.json"); do
        NFS_EXPORTS="${NFS_EXPORTS} ${ip}(rw,sync,no_subtree_check,no_root_squash)"
    done

    # 6a: Set up NFS server on master (RHEL: nfs-utils, nfs-server)
    echo -e "${YELLOW}Setting up NFS server on master...${NC}"
    ssh "${LME_USER}@${MASTER_IP}" "sudo dnf install -y nfs-utils"
    ssh "${LME_USER}@${MASTER_IP}" "sudo mkdir -p ${MASTER_SNAPSHOT_STORAGE} /srv/es-snapshots && sudo chmod 777 ${MASTER_SNAPSHOT_STORAGE} /srv/es-snapshots"
    # RHEL's root LV is small; back the exported path with the larger /var LV.
    ssh "${LME_USER}@${MASTER_IP}" "mountpoint -q /srv/es-snapshots || sudo mount --bind ${MASTER_SNAPSHOT_STORAGE} /srv/es-snapshots"
    ssh "${LME_USER}@${MASTER_IP}" "grep -q '${MASTER_SNAPSHOT_STORAGE} /srv/es-snapshots' /etc/fstab || echo '${MASTER_SNAPSHOT_STORAGE} /srv/es-snapshots none bind 0 0' | sudo tee -a /etc/fstab"
    ssh "${LME_USER}@${MASTER_IP}" "echo '${NFS_EXPORTS}' | sudo tee /etc/exports"
    ssh "${LME_USER}@${MASTER_IP}" "sudo exportfs -ra && sudo systemctl enable --now nfs-server"
    # If firewalld is running, allow NFS clients to reach the server
    ssh "${LME_USER}@${MASTER_IP}" 'if sudo systemctl is-active --quiet firewalld 2>/dev/null; then sudo firewall-cmd --permanent --add-service=nfs && sudo firewall-cmd --permanent --add-service=mountd && sudo firewall-cmd --permanent --add-service=rpc-bind && sudo firewall-cmd --reload; fi'
    ...
    for node_info in $ALL_NODE_IPS; do
        ...
        if [ "$node_priv_ip" = "$MASTER_PRIVATE_IP" ]; then
            ...
            ssh "${LME_USER}@${node_pub_ip}" "sudo mkdir -p /mnt/es-snapshots /srv/es-snapshots ${MASTER_SNAPSHOT_STORAGE}"
            ...
            ssh "${LME_USER}@${node_pub_ip}" "sudo chmod 777 ${MASTER_SNAPSHOT_STORAGE} /srv/es-snapshots /mnt/es-snapshots && sudo chcon -Rt container_file_t ${MASTER_SNAPSHOT_STORAGE} /srv/es-snapshots /mnt/es-snapshots"
        ...
        ssh "${LME_USER}@${node_pub_ip}" "sudo dnf install -y nfs-utils"
...
echo ""
echo -e "${GREEN}=== Cluster setup complete (RHEL 9) ===${NC}"
echo ""
