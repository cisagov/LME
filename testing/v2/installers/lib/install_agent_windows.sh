...
./run_elevated_powershell.sh "elastic-agent-8.18.3-windows-x86_64/elastic-agent-8.18.3-windows-x86_64/elastic-agent install --non-interactive --force --url=https://${HOST_IP}:$PORT --insecure --enrollment-token=${ENROLLMENT_TOKEN}"
...
#enrollment_output=$(./run_elevated_powershell.sh "./elastic-agent-8.18.3-windows-x86_64/elastic-agent-8.18.3-windows-x86_64/elastic-agent enroll --force --insecure --url=https://${HOST_IP}:$PORT --enrollment-token=${ENROLLMENT_TOKEN} ")
#
