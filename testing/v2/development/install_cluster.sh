...
echo -e "  docker exec $MASTER_CONTAINER bash -c 'ELASTIC_ID=\$(sudo -i podman secret ls | grep elastic | awk \"{print \\\$1}\") && curl -k -u \"elastic:\$(sudo -i ansible-vault view /etc/lme/vault/\$ELASTIC_ID)\" https://localhost:9200/_cluster/health?pretty'"
echo ""
