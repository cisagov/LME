...
def test_elastic_agent_logs_search(es_host, es_port, username, password):
    ...
    for x in range(len(data[rootKey]["hits"])):
        ...
        assert "source" in data[rootKey]["hits"][x]["_source"]["log"]
        ...
        assert data[rootKey]["hits"][x]["_source"]["host"]["hostname"]=="lme-fleet-server"
...
def test_elastic_indices(es_host, es_port, username, password):
    ...
    assert ("open .internal.alerts-transform.health.alerts-default" in response.text)              
    ...
    assert ("open .internal.alerts-default.alerts-default" in response.text)                     
    ...
    assert ("open .internal.alerts-ml.anomaly-detection-health.alerts-default" in response.text)   
    ...
    assert ("open .internal.alerts-observability.logs.alerts-default" in response.text)            
